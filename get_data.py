...
def fetch_poster(movie):
    ...
    country_origin = movie[2] if isinstance(movie, tuple) and len(movie) > 2 else "Japan"
    
    results = search_api("movie", movie_name, movie_year)
    if results:
        poster_path = results[0].get("poster_path")
        if poster_path:
            return (movie_name, movie_year, f"{IMAGE_URL}{poster_path}", "movie", country_origin)
            
    results = search_api("tv", movie_name, movie_year)
    if results:
        poster_path = results[0].get("poster_path")
        if poster_path:
            return (movie_name, movie_year, f"{IMAGE_URL}{poster_path}", "tv", country_origin)
        
    return (movie_name, movie_year, None, None, country_origin)
...
def Load_images():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor: 
        ...
        for era, fut in futures:
            movie_name, movie_year, poster_url, ent_type, country = fut.result()
            all_posters[era].append((movie_name, movie_year, poster_url, ent_type, country))
    sort_posters(all_posters)

def sort_posters(all_posters):
    for era, posters in all_posters.items():
        posters.sort(key=lambda x: (x[1] is None, x[1] if x[1] is not None else 0, x[0]))
    return all_posters

def fetch_movie_data(movie_title, movie_year, entetainment_type, country_origin):
    ...
    movie_data["trailer"] = get_yt_trailer_link(movie_title, movie_year, country_origin)
...
def get_yt_trailer_link(movie_title, movie_year, country_origin):
    query = f"{movie_title} {movie_year} {country_origin} trailer"
